...
--
-- Хост: 127.0.0.1:3306
-- Время создания: Апр 09 2025 г., 23:48
...
-- Процедуры
--
CREATE PROCEDURE `CalculateTaxBaseOrNDFL` (IN `p_employee_id` INT, IN `operation` INT, IN `consider_veteran` BOOLEAN, IN `consider_children` BOOLEAN, OUT `result` DECIMAL(8,2))   BEGIN
    IF(operation = 1) THEN
...
CREATE PROCEDURE `WriteTaxBaseOrNDFL` (IN `p_employee_id` INT, IN `operation` INT, IN `consider_veteran` BOOLEAN, IN `consider_children` BOOLEAN)   BEGIN
...
-- Функции
--
CREATE FUNCTION `CalculateNDFL` (`p_employee_id` INT, `consider_veteran` BOOLEAN, `consider_children` BOOLEAN) RETURNS DECIMAL(8,2)  BEGIN
...
CREATE FUNCTION `CalculateTaxBase` (`p_employee_id` INT, `consider_veteran` BOOLEAN, `consider_children` BOOLEAN) RETURNS DECIMAL(8,2) READS SQL DATA BEGIN
...
CREATE TABLE `accrual` (
  ...
  `date_accrual` date DEFAULT (curdate()),
...
CREATE TABLE `proc_logs` (
  ...
  `amount` decimal(8,2) NOT NULL,
  `operation_datetime` datetime DEFAULT (now())
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_0900_ai_ci;
